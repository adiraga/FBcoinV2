...
void StakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("fbcoin-stakeminer");
...
void static FBcoinMiner(CWallet *pwallet)
{
    printf("FBcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fbcoin-miner");
    ...
    try { loop {
        ...
        {
            printf("FBcoinMiner : no more PoW blocks\n");
            return;
        }

        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        printf("Running FBcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FBcoinMiner terminated\n");
        throw;
    }
}

void GenerateFBcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FBcoinMiner, pwallet));
}
