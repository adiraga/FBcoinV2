...
// Copyright (c) 2014 The FBcoin developers
...
#if defined(NDEBUG)
# error "FBcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x383cf428a71c5ec42c68a2cbdebcf6b98c42017b98f72712a8d35a50d52bd254");
uint256 hashGenesisBlockTestNet("0x46db8d519c64586e3f0bf16153f61fdcf1d30b71ab159bf4a5b46dbcbc79d814");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FBcoin: starting difficulty is 1 / 2^12
...
unsigned int nStakeMaxAge = 100 * 24 *  60 * 60; // 100 days
...
{
    // FBcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 0) {
        ...
        // Premine: First 10 block are 15,000,000 XFB (75% of the total coin to redistribute)
        nSubsidy = 15000000 * COIN;
    } else if (nHeight < 10000) {
        // Bonus reward for block 10-10000 of 100 coins
        nSubsidy = 100 * COIN;
        return nSubsidy + nFees;
    }else if (nHeight < 34000) {
             // Bonus reward for block 10-34000 of 1000 coins to make the 175M
             nSubsidy = 1000 * COIN;
         }
    return nSubsidy + nFees;
}
...
int64 GetProofOfStakeReward(int64 nCoinAge, int64 nFees, uint256 prevHash, int nHeight)
{
    ...
    int factor = 10;
    bool superBlockEnabled = false;
    int superBlockFactor = 100;
    if(nHeight >= 110000){
        factor = 100;
    }
    if(nHeight > 150000){
    ...
    int rand = generateMTRandom(seed, factor-1)+1;

    if(superBlockEnabled){
        int superBlockRand = generateMTRandom(seed, 65280);
        if(superBlockRand==0){
            rand = superBlockFactor;
        }
    }

    nSubsidy = nCoinAge * (rand * CENT) * 33 / (365 * 33 + 8);
...
const int64 nTargetSpacing = 60; // FBcoin: 1 minute block
static const int64 nTargetTimespan = 60 * 60; // FBcoin: Not used
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    else if (IsProofOfStake())
    {
        ...
        int64 nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees, prevHash,pindex->nHeight);
...
{
    // FBcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        printf("block.nNonce = %u \n", block.nNonce);
			  block.print();
        assert(block.hashMerkleRoot == uint256("0x2d949581d889c19866c6b00936f10453fcb66f847ac4cf28bbf6db639a231d5b"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xfb, 0xb6, 0xdb }; // FBcoin: increase each by adding 2 to bitcoin's value.
